...
@Data
public class AppGiftV2ForPublicWriteOff
{
    ...
    {
        if (status == CardStatus.USED) return "已使用";
        if (Boolean.TRUE.equals(invalid)) return "已作废";
        if (status == CardStatus.EXPIRED) return "已过期";
        return "未使用";
    }
}
