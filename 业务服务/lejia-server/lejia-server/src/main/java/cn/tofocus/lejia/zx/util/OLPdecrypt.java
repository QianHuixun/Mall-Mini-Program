...
public class OLPdecrypt
{
	...
	{
		String path = "C:\\Users\\Administrator\\Desktop\\rrrrr.txt";
		String xmlStr = Utils.readFileContent(path);
		System.out.println(xmlStr);
		try {
			Document doc = DocumentHelper.parseText(xmlStr);
			Element root = doc.getRootElement();
			List<Element> list = root.elements("ContrInfoList");
			for (Element le : list) {
				System.out.println(le.elementTextTrim("Pay_Typ_Encd"));
				System.out.println(le.elementTextTrim("Pay_Type_Fee_Rate"));
			}
			
			//System.out.println(e.elementTextTrim("chnl_mercht_id"));
		} catch (Exception e) {
			e.printStackTrace();
		}
