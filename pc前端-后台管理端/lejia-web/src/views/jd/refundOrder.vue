  ...
  <div class="table-container">
    ...
    <div class="table-box">
      <el-table :data="tableData" :loading="loading" border style="width: 100%">
        ...
        <el-table-column label="京东订单编号" prop="jdOrderId" min-width="120"></el-table-column>
        <el-table-column label="商品价格" prop="goodsAmt" width="120"></el-table-column>
        <el-table-column label="邮费" prop="postage" width="120"></el-table-column>
        ...
        <el-table-column label="邮费退款总价" prop="refundPostage" width="120"></el-table-column>
        ...
        <el-table-column label="退款原因" min-width="160px" prop="reason"></el-table-column>
        <el-table-column label="申请时间" prop="createdTime" min-width="150px" ></el-table-column>
        <el-table-column label="操作" width="150" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="handleRefundWX(scope.row)" v-if="scope.row.againRefund">
              微信退款
            </el-button>
    ...
    <refund-again ref="RefundAgain" @refresh="getData"></refund-again>
  </div>
</template>
<script>
  import qs from "qs";
  import RefundDetail from "./RefundOrder/RefundDetail.vue";
  import RefundAgain from "./RefundOrder/RefundWXAgain.vue"
  export default {
    data() {
      return {
        downLoading: false,
        ...
        statusList: [{
          ...
          {
            pkey: "JD_PENDING_APPROVAL",
            name: "申请中待审核"
          },
          {
            pkey: "JD_APPROVAL_ACCEPTED",
            name: "审核完成待收货"
          },
          {
            pkey: "JD_APPROVAL_REJECTED",
            name: "审核不通过"
          },
          {
            pkey: "JD_RECEIPTED",
          ...
          {
            pkey: "JD_PROCESSED_SUCCESS",
            name: "处理完成"
          },
          {
            pkey: "JD_PROCESSED_FAILED",
            name: "处理失败"
          },
          {
            pkey: "JD_PENDING_CONFIRM",
            name: "待用户确认"
          },
          {
            pkey: "JD_CONFIRMED",
    ...
    methods: {
      ...
      getData: function () {
        ...
        axios
          .post(api.jd.refundQuery, qs.stringify(params))
          .then(response => {
      ...
      /**
       *  微信退款：当系统退款失败可手动触发微信退款
       */
      handleRefundWX:function(row) {
        this.$refs.RefundAgain.show(row);
      },
      /**列表导出 */
      handleImportExcel() {
        const params = {
        ...
        axios
          .post(api.jd.refundExport, qs.stringify(params), {
            headers: {
          ...
          .then(res => {
            ...
            reader.addEventListener("loadend", function (e) {
              if (e.target.result.indexOf("result") > 0) {
                ...
                var fileName = headersFileName && headersFileName.length != 0 ? decodeURI(headersFileName[1]) : '京东退款订单.xlsx'
