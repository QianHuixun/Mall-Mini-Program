  ...
  <div class="table-container">
      ...
      <div class="search-box-button">
        <div class="count-container">
          <div class="count-box">
            ...
            <span class="count-num-xs">{{ numData.amtoSum }}</span>
          </div>
          <div class="count-box">
            <span class="count-tit">配送费合计：</span>
            <span class="count-num-xs">{{ numData.postageSum }}</span>
          </div>
          <div class="count-box">
            <span class="count-tit">积分总价：</span>
            <span class="count-num-xs">{{ numData.pointnSum }}</span>
          </div>
           <div class="count-box">
            <span class="count-tit">合计金额：</span>
            <span class="count-num-xs">{{ numData.amtnSum }}</span>
          </div>
        </div>
    ...
    <div class="table-box">
      <el-table :data="tableData" :loading="loading" border style="width: 100%" >
        ...
        <el-table-column label="订单笔数" prop="orderCount"></el-table-column>
        <el-table-column label="销售数量" prop="goodsCount"></el-table-column>
        <el-table-column label="积分总价" prop="pointnSum"></el-table-column>
        <el-table-column label="积分退款" prop="refundPoint"></el-table-column>
        <el-table-column label="商品总价" prop="amtoSum"></el-table-column>
        <el-table-column label="商品退款" prop="refundAmt"></el-table-column>
        <el-table-column label="配送费" prop="postageSum"></el-table-column>
        <el-table-column label="配送费退款" prop="refundPostage"></el-table-column>
        <el-table-column label="合计金额" prop="amtnSum"></el-table-column>
      </el-table>
      <!-- 页码 -->
...
export default {
  data() {
    return {
      ...
      selectOptions: [{ name: "供应商", key: "supplierName" }],
      tableData: [],
  ...
  methods: {
    ...
    getData: function () {
      ...
      axios
        .post(api.data.querySupplierSales, qs.stringify(params), {
          headers: {
    ...
    getDataNum: function () {
      ...
      axios
        .post(api.data.querySupplierSalesSum, qs.stringify(params), {
          headers: {
            Authorization: this.$store.state.token,
          },
        })
    ...
    handleImportExcel() {
      ...
      axios
        .post(api.data.querySupplierSalesExport, qs.stringify(params), {
          headers: {
            Authorization: this.$store.state.token,
          },
          responseType: "blob",
          ...
          reader.addEventListener("loadend", function (e) {
            if (e.target.result.indexOf("result") > 0) {
              ...
              if (!!window.ActiveXObject || "ActiveXObject" in window) {
                window.navigator.msSaveOrOpenBlob(blob, `${"供应商统计"}.xlsx`);
              } else {
                const link = document.createElement("a");
                link.style.display = "none";
                link.href = URL.createObjectURL(blob);
                link.setAttribute("download", `${"供应商统计"}.xlsx`);
