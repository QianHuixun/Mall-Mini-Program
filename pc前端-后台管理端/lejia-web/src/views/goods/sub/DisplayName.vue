...
export default {
  ...
  methods: {
    ...
    getData() {
      axios.post(api.goods.getDisplayName, this.$qs.stringify({mType: this.mType}))
        .then(res => {
    ...
    handleSubmit() {
      ...
      const params = {
        mType: this.mType,
        displayName: this.inputModel.name
      };
      axios
        .post(api.goods.setDisplayName, this.$qs.stringify(params))
        .then((res) => {
          if (res) {
