...
echo Starting JMeter P-01 UI...
echo JMeter: %JMETER_CMD%
echo Test plan: %ROOT_DIR%\03-p01-home-performance.jmx
pushd "%ROOT_DIR%"
if errorlevel 1 goto path_failed
call "%JMETER_CMD%" -t "%ROOT_DIR%\03-p01-home-performance.jmx" -q "%ROOT_DIR%\config\p01-performance.properties" -j "%ROOT_DIR%\results\p01-ui.log"
set "EXIT_CODE=%ERRORLEVEL%"
popd
if "%EXIT_CODE%"=="0" goto success
echo ERROR: JMeter exited with code %EXIT_CODE%.
echo Log: %ROOT_DIR%\results\p01-ui.log
pause
