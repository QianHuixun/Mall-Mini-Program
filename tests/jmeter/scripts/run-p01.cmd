...
set "STAMP=%STAMP: =0%"
set "JTL=%ROOT_DIR%\results\p01-%STAMP%.jtl"
set "REPORT=%ROOT_DIR%\results\p01-report-%STAMP%"

call "%JMETER_CMD%" -n ^
  -t "%ROOT_DIR%\03-p01-home-performance.jmx" ^
  -q "%ROOT_DIR%\config\p01-performance.properties" ^
  -l "%JTL%" ^
  -e -o "%REPORT%"

if errorlevel 1 (
  echo P-01 执行失败，请检查上方日志和断言。
) else (
  echo P-01 执行完成。
  echo JTL: %JTL%
